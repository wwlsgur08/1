...
let starSizes = Array(traits.length).fill(0);
...
function createStars() {
    ...
    for (let i = 0; i < traits.length; i++) {
        ...
        // 랜덤 오프셋 (-15% ~ +15% 정도)
        const offsetX = (Math.random() - 0.5) * 0.3 * cellWidth;
        const offsetY = (Math.random() - 0.5) * 0.3 * cellHeight;
...
function updateStarPositions() {
    ...
    stars.forEach((star, i) => {
        ...
        // 저장된 오프셋 비율 적용
        const offsetRatioX = (starPositions[i].x / starPositions[i].originalWidth) - (col + 0.5) / 7;
        const offsetRatioY = (starPositions[i].y / starPositions[i].originalHeight) - (row + 0.5) / 7;
        
        // 새 오프셋 계산
        const offsetX = offsetRatioX * containerWidth;
        const offsetY = offsetRatioY * containerHeight;
...
function resetConstellation() {
    ...
    // 별 크기 상태 배열 초기화
    starSizes = Array(traits.length).fill(0);
...
// 초기 컨테이너 크기 저장 (리사이징을 위해)
function storeOriginalContainerSize() {
    const container = document.querySelector('.star-container');
    const containerWidth = container.clientWidth;
    const containerHeight = container.clientHeight;
    
    // 각 별 위치에 원래 컨테이너 크기 정보 추가
    starPositions.forEach(pos => {
        pos.originalWidth = containerWidth;
        pos.originalHeight = containerHeight;
    });
}

// 페이지 로드 완료 후 실행할 추가 초기화
window.addEventListener('load', () => {
    storeOriginalContainerSize();
});